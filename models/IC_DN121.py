...
class IC_DenseNet121:
    def get_densenet(self):
        densenet = DenseNet121(weights='imagenet', include_top=False,
                               input_shape=(None, None, 3))
        i = 0
        while(True):
            if densenet.layers[i].name == 'pool3_conv':
                break
            i += 1
    ...
    def get_low_level_features(self):
        ...
        densenet = self.get_densenet()(preprocess)
        new_layers = self.get_dense_block(K.int_shape(densenet)[-1], 0, 8)(densenet)
        return Model(inputs=input_layer,
                     outputs=new_layers,
