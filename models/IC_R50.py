...
class IC_ResNet50:
    ...
    def get_low_level_features(self):
        ...
        reduce_a = Conv2D(filters=128, kernel_size=1, strides=1, padding='same',
                        kernel_initializer='he_uniform',
                        bias_initializer='he_uniform',
                        name='{}_reduce_a'.format(self.low_name))(resnet)
        reduce_b = Conv2D(filters=128, kernel_size=3, strides=2, padding='same',
                        kernel_initializer='he_uniform',
                        bias_initializer='he_uniform',
                        name='{}_reduce_b'.format(self.low_name))(reduce_a)
        new_layers = self.get_dense_block(K.int_shape(reduce_b)[-1], 0)(reduce_b)
        return Model(inputs=input_layer,
                     outputs=new_layers,
