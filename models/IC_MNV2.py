...
class IC_MobileNetV2:
    def get_mobilenet(self):
        mobilenet = MobileNetV2(weights='imagenet', include_top=False,
                                input_shape=(None, None, 3))
        i = 0
        while(True):
            if mobilenet.layers[i].name == 'block_4_add':
                break
            i += 1
        model = Model(inputs=mobilenet.layers[0].input,
                      outputs=mobilenet.layers[i].output,
                      name='mobilenet')
    ...
    def get_low_level_features(self):
        ...
        preprocess = Lambda(lambda x: (x / 128.0) - 1.0,
                            name='{}_pre'.format(self.low_name))(duplicate)
        mobilenet = self.get_mobilenet()(preprocess)
        new_layers = self.get_dense_block(K.int_shape(mobilenet)[-1], 0)(mobilenet)
        return Model(inputs=input_layer,
                     outputs=new_layers,
